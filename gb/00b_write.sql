...
-- write to rBANK (0xFF50) unmaps the boot ROM, matching real hardware. A write to
-- 0xFF00 (joypad select bits) stores the recomputed combined byte instead of the raw
-- written value, so gb_read8 can stay a plain array read there - see gb_joypad_byte's
-- comment in 00_helpers.sql.
CREATE OR REPLACE FUNCTION gb_write8 AS (s, addr, val) ->
    gb_mk(
        multiIf(addr = 0xFF44, gb_mem(s),
                addr = 0xFF00, gb_set(gb_mem(s), 0xFF00 + 1, gb_joypad_byte(val, gb_buttons(s))),
                gb_set(gb_mem(s), CAST(addr AS UInt32) + 1, val)),
        gb_boot_rom(s),
        if(addr = 0xFF50 AND val != 0, CAST(0 AS UInt8), gb_boot_active(s)),
        gb_a(s), gb_b(s), gb_c(s), gb_d(s), gb_e(s), gb_h(s), gb_l(s), gb_f(s),
        gb_sp(s), gb_pc(s), gb_ime(s), gb_cycles(s), gb_buttons(s)
    );
...
CREATE OR REPLACE FUNCTION gb_with_regs AS (s, a, b, c, d, e, h, l, f, sp, pc, ime, cycles) ->
    gb_mk(gb_mem(s), gb_boot_rom(s), gb_boot_active(s), a, b, c, d, e, h, l, f, sp, pc, ime, cycles, gb_buttons(s));
